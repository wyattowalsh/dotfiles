...
test_help_and_version() {
    ...
    assert_contains "$version_out" "freshen 1.7.1" "version reports new release"
}
...
test_help_completion_and_version_metadata_parity() {
    ...
    assert_contains "$FRESHEN_FILE" "freshen v1.7.1" "header version matches release"
    ...
    assert_contains "$out" "freshen 1.7.1 (2026-07-04)" "version output matches release metadata"
}
...
test_cache_mixed_results() {
    ...
    run_freshen "$out" --yes --no-mas --no-cleanup --no-color
