...
Human operator runbooks live under `docs/content/docs/`. Hub slugs, sidebar order, and `just docs-ci` are the docs gates.

## Requirements

### Requirement: Docs Hub Single Source Of Truth

Operator hub content for the landing page-list and the docs index hub SHALL be derived from `docs/lib/hub-manifest.json` (via `docs/lib/sections.ts`). Sidebar order SHALL remain defined in `docs/content/docs/meta.json`.

#### Scenario: Landing and index share hub data

- **WHEN** hub content is rendered on `/` or `/docs`
- **THEN** page-list and hub-table rows are derived from `docs/lib/hub-manifest.json`
...
- **THEN** frozen dependency install, docs typecheck, docs production build, CSS health, docs-sensitive scanning, and docs-hub-parity succeed
...
Subsystem README files MAY exist when they are short and non-authoritative. They SHALL NOT replace `docs/content/docs/` as the operator runbook SSOT.
