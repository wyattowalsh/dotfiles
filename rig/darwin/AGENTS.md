...
`rig/darwin/` — nix-darwin + Home Manager flake for Apple Silicon macOS (`w4w-mbp` host).

## Rules

- Host-specific values live in `rig/darwin/hosts/` — not in portable `rig/home/` templates
- Commit `rig/darwin/flake.lock`; apply path requires it (`rig/bootstrap/macos.sh`)
...
```bash
nix flake lock ./rig/darwin          # generate lock before first apply
