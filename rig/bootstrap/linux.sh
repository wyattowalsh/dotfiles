...
ensure_apt_updated() {
  ...
  if [ "$DRY_RUN" -eq 1 ] || [ "$SMOKE_CHECK" -eq 1 ]; then
    skip_action "Would update apt package index (privilege on apply)"
    return 0
  fi
...
install_apt_tools() {
  ...
  if [ "$DRY_RUN" -eq 1 ] || [ "$SMOKE_CHECK" -eq 1 ]; then
    skip_action "Would install apt packages: ${missing[*]} (privilege on apply)"
    return 0
  fi
...
install_tar_binary_from_github() {
  ...
  if [ "$DRY_RUN" -eq 1 ] || [ "$SMOKE_CHECK" -eq 1 ]; then
    skip_action "Would install ${binary} from ${repo} (privilege on apply)"
    return 0
  fi
...
install_deb_from_github() {
  ...
  if [ "$DRY_RUN" -eq 1 ] || [ "$SMOKE_CHECK" -eq 1 ]; then
    skip_action "Would install ${check_cmd} from ${repo} (privilege on apply)"
    return 0
  fi
...
install_go() {
  ...
  if [ "$DRY_RUN" -eq 1 ] || [ "$SMOKE_CHECK" -eq 1 ]; then
    skip_action "Would install Go for ${ARCH_NAME} (privilege on apply)"
    return 0
  fi
...
create_symlinks() {
  ...
  link_file "$DOTS_DIR/p10k.zsh" "$HOME/.p10k.zsh"
